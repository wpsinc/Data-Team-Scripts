...
base_path = os.path.join(
    home_dir, "OneDrive - Arrowhead EP/GSC Karen Bacon Project/Accounts Payable"
)
output_path = os.path.join(
    home_dir,
    "OneDrive - Arrowhead EP/GSC Karen Bacon Project/Ready to Submit",
)
...
Voucher = os.path.join(base_path, "By Voucher")

# Check if Mega exists
if not os.path.exists(Voucher):
    print(f"Error: The path {Voucher} does not exist for this user.")
    exit()

# Navigate to folder containing inventory files
Items = os.path.join(base_path, "By Item")

# Check if Mega exists
if not os.path.exists(Items):
    print(f"Error: The path {Items} does not exist for this user.")
    exit()
...
Voucher_file = os.path.join(Voucher, "Accounts Payable Voucher")
Voucher_encoding = find_encoding(Voucher_file)
VoucherDF = pd.read_csv(
    Voucher_file, dtype=str, encoding=Voucher_encoding, sep="\t", engine="python"
)
VoucherDF.rename(
    columns={
